...
if ! sudo -E apt-get install -y apt-transport-https ca-certificates curl gnupg lsb-release; then
...
# Add docker GPG key
if ! curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo -E gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg; then
...
if ! [ -z "$swap_status" ]; then
    ...
    # Turn off swap
    if ! sudo -E swapoff -a; then
        echo "Error: Failed to turn off swap"
        exit 1
    fi

    # Disable swap completely
    if ! sudo -E sed -i -e '/swap/d' /etc/fstab; then
        echo "Error: Failed to disable swap in fstab"
        exit 1
    fi

    # Verify that swap is disabled
    if ! swap_status=$(swapon --show); then
